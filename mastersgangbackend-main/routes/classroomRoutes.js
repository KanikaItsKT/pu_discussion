...
const mailer = async (receiverEmail, code) => {
    ...
    let info = await transporter.sendMail({
        from: "Panjab University Online",
        to: receiverEmail,
        subject: "OTP for Campus Sync",
    ...
    try {
      ...
      const studentEmails = classroom.students || [];
        if (studentEmails.length === 0) {
            return res.json({ success: true, data: [] }); // Return an empty array if no students
        }

        // Fetch student details from the User collection
        const studentsDetails = await User.find(
            { email: { $in: studentEmails }, role: 'student' },
            'name email' // Select only the name and email fields
        );
