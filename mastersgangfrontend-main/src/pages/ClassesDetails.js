...
const ClassesDetails = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/class/${classid}/students`,
      ...
      if (response.ok) {
        setStudents(data.data); // Update the state with the fetched students
      } else {
  ...
  {students.length > 0 ? (
    <table className="table table-striped">
      <thead>
        <tr>
          <th colSpan="3" className="text-center">Total Students: {students.length}</th>
        </tr>
        <tr>
          <th>Sr. No.</th>
          <th>Name</th>
